...
import io.github.blackfishlabs.domain.repository.NFeDAO;
...
public class NFeController {
    ...
    public String cancel(FiscalEventCancellationDTO dto) throws Exception {
        ...
        List<NFeEntity> filterNFe = new NFeDAO().
                filter("from NFeEntity where key = '" + dto.getKey() +
        ...
        if (nFeEntity.isPresent()) {
            ...
            if (event.isPresent()) {
                ...
                if (proc.isPresent()) {
                    ...
                    new NFeDAO().update(nFeEntity.get());
